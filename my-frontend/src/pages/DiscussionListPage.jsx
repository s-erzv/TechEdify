            ...
            <div className="flex-grow p-6 bg-[#F9F9FB] rounded-xl min-h-[calc(100vh-80px)]">
                <header className="mb-8 p-4 bg-white rounded-xl shadow-sm flex items-center justify-between">
                    <h1 className="text-3xl font-bold text-gray-900 flex items-center">
                        <ChatBubbleLeftRightIcon className="h-8 w-8 mr-3 text-purple-600" /> Forum Diskusi
                    </h1>
                    <button
                        onClick={handleCreateNewDiscussion}
                        className="px-5 py-2 bg-purple-600 text-white rounded-md font-semibold hover:bg-purple-700 transition-colors flex items-center"
                    >
                ...
                <div className="mb-6 p-4 bg-white rounded-xl shadow-sm flex flex-col sm:flex-row gap-4">
                    ...
                    <div className="relative">
                        <select
                            className="w-full sm:w-auto pl-4 pr-10 py-2 border border-gray-300 rounded-md appearance-none focus:ring-purple-500 focus:border-purple-500 bg-white"
                ...
                <div className="space-y-6">
                    {discussions.length > 0 ? (
                        discussions.map((discussion) => (
                            <div 
                                key={discussion.id} 
                                className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 border border-gray-200 cursor-pointer"
                                onClick={() => handleViewDiscussion(discussion.id)}
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <h2 className="text-xl font-semibold text-gray-800 line-clamp-1">{discussion.title}</h2>
                                ...
                                <p className="text-gray-600 text-sm mb-3 line-clamp-2">{discussion.content || "Tidak ada konten."}</p>
                                <div className="flex items-center text-xs text-gray-500">
                                    ...
                                    <span>Diposting oleh <span className="font-medium text-gray-700">{discussion.author_name}</span></span>
                                    ...
                                    {discussion.module_title && (
                                        <span className="ml-3 px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">
                                            {discussion.module_title}
                                        </span>
                                    )}
